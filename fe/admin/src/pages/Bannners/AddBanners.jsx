...
const API_ADD_ACTOR = "http://filmgo.io.vn/api/banners/create";
...
const AddBanners = () => {
  ...
  const handleSubmit = async (values) => {
    ...
    formData.append("image", file); // Gửi file lên API
    ...
    nav("/admin/list-banners");
  };

  return (
    <Card title="Thêm Banner mới" style={{ maxWidth: 500, margin: "auto" }}>
      <Form form={form} onFinish={handleSubmit} layout="vertical">
        <Form.Item
          label="Tên ảnh"
          name="name"
          rules={[{ required: true, message: "Vui lòng nhập tên banners !" }]}
        >
          <Input placeholder="Nhập tên diễn viên" />
        </Form.Item>

        <Form.Item
          label="Ảnh banner (JPG, PNG)"
          name="image"
